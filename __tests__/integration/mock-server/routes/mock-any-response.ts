...
export function mockAnyResponse(
  ...
  if (
    ...
    const prefixLen = parts[0].length;
    const postfixLen = parts[1].length;
    // console.log('slicing chars', prefixLen, postfixLen, c.req.path);
    const idStr = c.req.path.slice(prefixLen, -postfixLen);
  ...
  if (
    ...
    body = applyPagination(body, acceptedResponse.syncables[0], c.req.query(), schema);
  }
  c.status(statusCode);
