...
const handleObjectSchema = (
  ...
  if ('properties' in schema && schema.properties) {
    ...
    const limit =
      level > 3
        ? Math.min(MAX_PROPERTIES, propertyNames.length)
    ...
    for (let i = 0; i < limit; i++) {
      ...
      const propertyXmlName =
        options?.xml && 'xml' in propertySchema
...
const handleArraySchema = (
  ...
  const itemsXmlTagName =
    items && typeof items === 'object' && 'xml' in items
      ? items.xml?.name
      : undefined;
  const wrapItems = !!(
    options?.xml &&
  ...
  if (schema.example !== undefined) {
    return cache(
      schema,
      wrapItems
        ? { [itemsXmlTagName as string]: schema.example }
        : schema.example,
    );
  }
  ...
  if (items && typeof items === 'object') {
    if (Array.isArray(items.allOf) && items.allOf.length > 0) {
      ...
      if (
        ...
        return cache(
          schema,
          wrapItems ? [{ [itemsXmlTagName as string]: merged }] : [merged],
        );
      }
      ...
      return cache(
        schema,
        wrapItems
          ? (examples as unknown[]).map((e) => ({
              [itemsXmlTagName as string]: e,
            }))
          : examples,
      );
    }
    ...
    if (union && union.length > 0) {
      ...
      return cache(
        schema,
        wrapItems ? [{ [itemsXmlTagName as string]: ex }] : [ex],
      );
    }
  }
  ...
  const isObject =
    items &&
    typeof items === 'object' &&
    (('type' in items && items.type === 'object') || 'properties' in items);
  const isArray =
    items &&
    typeof items === 'object' &&
    (('type' in items && items.type === 'array') || 'items' in items);

  if (
    items &&
    typeof items === 'object' &&
    (('type' in items && items.type) || isObject || isArray)
  ) {
  ...
  if ('type' in schema && schema.type && !Array.isArray(schema.type)) {
    switch (schema.type) {
      ...
      case 'boolean':
        return true;
      case 'integer':
        return 'minimum' in schema && typeof schema.minimum === 'number'
          ? schema.minimum
          : 1;
      case 'number':
        return 'minimum' in schema && typeof schema.minimum === 'number'
...
export const getExampleFromSchema = (
  ...
  if ('x-variable' in _schema && _schema['x-variable']) {
    ...
    if (value !== undefined) {
      ...
      if (
        'type' in _schema &&
        (_schema.type === 'number' || _schema.type === 'integer')
      ) {
        seen.delete(targetValue);
  ...
  if (
    'properties' in _schema ||
    ('type' in _schema && _schema.type === 'object')
  ) {
  ...
  const primitive = getPrimitiveValue(
    _schema,
    makeUpRandomData,
    options?.emptyString,
  );
  ...
  if (Array.isArray(_schema.allOf) && _schema.allOf.length > 0) {
    ...
    for (const item of items) {
      const ex = getExampleFromSchema(
        item as OpenAPIV3_1.SchemaObject,
        options,
        {
          level: level + 1,
  ...
  const unionPrimitive = getUnionPrimitiveValue(
    _schema,
    makeUpRandomData,
    options?.emptyString,
  );
